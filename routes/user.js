...
module.exports = private => {
  ...
  router.post('/reset', (req, res) => {
    ...
    User.findOne({ emailAddress })
      .then(user => {
         ...
         */
        ResetLink.findOne({ email: emailAddress })
          .then(l => {
            if (l.stillValid)
              res
                .status(400)
                .json({
                  error:
                    'Link Already Exists. Please Use that one. Only one can be valid at the time',
                });
            new ResetLink({ email: emailAddress })
              .save()
              .then(link => {
                if (!link) {
                  log.error(
                 ...
                 */
                const resetReferral = `http://localhost:3201/users/reset/${
                  link.id
                }`;
              ...
              .catch(err =>
                res
                  ...
                  .json({
                    error: err,
                    message: 'issue creating new reset link',
                  }),
              );
          })
          .catch(err =>
            res
              ...
              .json({
                error: err,
                message: 'issue looking for existing links.',
              }),
          );
      })
      .catch(err =>
  ...
  router.get(
    ...
    isAuthAdmin,
    (req, res) => {
      Users.find({}, { password: -1 })
        .then(users => {
          if (!users)
            res
              .status(400)
              .json({ message: 'Seems to be no users in the database.' });
          res.json(users);
        })
