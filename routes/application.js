...
module.exports = (secret, public) => {
  ...
  router.get('/guest/list', (req, res) => {
    Application.find(
      {},
      'appName appOwner appDescription appIcon appBanner users permBannedUsers redirectURL tempBannedUsers',
    )
      .then(apps => {
        if (!apps)
          res
            .status(400)
            .json({ message: 'no apps currently in the database.' });
        res.json(apps);
      })
      .catch(err => {
  ...
  router.post(
    ...
    (req, res) => {
      const {
        errors,
        isValid,
      } = require('../validation/application').registerApp(req.body);
      ...
      Application.findOne({ appName: req.body.appName })
        ...
        .catch(err =>
          res.status(500).json({
            error: err,
            message: 'issue with finding application in database.',
          }),
        );
    },
  );
      ...
      Application.findById(appId)
        .then(app => {
          if (!app)
            res
              .status(500)
              .json({ message: 'issue locating app to add user to.' });
          app.users
            .push(userId)
            .save()
            .then(a => {
              if (!a)
                res.status(500).json({
                  message: 'issue saving app to database after user push.',
                });
              res.json(a);
            })
            .catch(err => res.status(500).json(err));
        ...
        .then(app => {
          if (!app)
            res.status(500).json({
              ...
              message: 'Issue with Retrieving application from database.',
            });
          User.findById(userId)
            .then(user => {
              if (!app)
                res.status(500).json({
                  error: 'Internal Server Error',
                  message: 'Issue retrieving user from database.',
                });
              const banned = {
                expires: banTime,
                user,
                ...
                .then(app => {
                  if (!app)
                    res.status(500).json({
                      ...
                      message: 'Issue saving application record to database.',
                    });
      ...
      Application.findById(appId)
        .then(app => {
          if (!app)
            res.status(500).json({
              error: 'Internal Server Error',
              message: 'Issue locating application in database',
            });
          User.findById(userId)
            .then(user => {
              if (!user)
                res.status(500).json({
                  error: 'Internal Server Error',
                  message: 'Issue locating user in database.',
                });
              app.permBannedUsers.push(user);
              app
                .save()
                .then(app =>
                  res.json({ message: 'Successfully banned user permenantly' }),
                );
            })
            .catch(err => res.status(500).json(err));
        })
  ...
  router.get('/bans/:appId/:userId', (req, res) => {
    const appId = req.params.appId;
    const userId = req.params.userId;
    Applicaiton.findById(appId)
      .then(app => {
        if (!app)
          res.status(500).json({ message: 'Issue finding App In database.' });
        if (app.tempBannedUsers.indexOf(userId) !== -1) {
          const banInfo = app.tempBannedUsers.filter(u => u.user === userId);
          res.json({
            notBanned: false,
            tempBanInfo: banInfo,
            message: `user is temporarily banned. Ban Expires: ${
              banInfo.expires
            }`,
          });
  ...
  router.get(
    ...
    (req, res) => {
      Application.find(
        { appOwner: req.user.id },
        'appName appDescription appIcon appBanner users permBannedUsers appOwner ownerDelegates',
      )
        .then(apps => {
          if (!apps)
            res.status(404).json({ error: 'No Apps from this user Found' });
          res.json(apps);
        })
        .catch(err =>
          res.status(500).json({
            error: err,
            message: 'issue finding apps in the database.',
          }),
        );
    },
  );
